defmodule Hush.ResolverTest do
  ...
  test "resolve() missing adapter" do
    ...
    assert Resolver.resolve(config) == {:error, %RuntimeError{message: "Elixir.Hush.Provider.DoesNotExist: Ran into an error: Provider Elixir.Hush.Provider.DoesNotExist is not available (nofile)"}}
  end
  ...
  test "resolve() required" do
    ...
    assert Resolver.resolve(config) == {:error, %ArgumentError{message: "Elixir.Hush.Provider.NotFound: Could not resolve required value from config key 'foo' provided by 'HUSH_UNKNOWN'"}}
  end
  ...
  test "resolve() with bad cast" do
    ...
    assert Resolver.resolve(config) == {:error, %ArgumentError{message: "Elixir.Hush.Provider.Echo: Could not convert config key 'foo' to 'integer' (possible sensitive value was hidden)"}}
  end
  ...
  test "resolve() with malformed provider implementation" do
    ...
    assert Resolver.resolve(config) == {:error, %RuntimeError{message: "Elixir.Hush.Provider.Malformed: Ran into an error: Unexpected format from provider: wrong return. Expected {:ok, value}, {:error, :not_found} or {:error, \"error\"}"}}
